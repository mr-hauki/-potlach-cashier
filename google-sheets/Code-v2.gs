/*
 * ПОТЛАЧ — безопасный импорт кассовых операций в Google Sheets, v2.
 *
 * ГАРАНТИИ:
 * - Supabase используется только для GET-запросов.
 * - Скрипт не изменяет и не удаляет операции в базе.
 * - Перед импортом создаётся резерв вкладки «02 — ПРОВЕРКА».
 * - Ручная проверка сохраняется по tx_id, а не по номеру строки.
 * - Нет блокирующих alert/prompt: результат всегда пишется в таблицу.
 */
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('ПОТЛАЧ')
    .addItem('1. Проверить соединение — быстро', 'testPotlachConnection')
    .addItem('2. Обновить операции — только чтение', 'importPotlachTransactions')
    .addToUi();
}

/**
 * Быстрый безопасный тест.
 * Получает максимум одну запись и не меняет рабочие вкладки.
 */
function testPotlachConnection() {
  const startedAt = new Date();
  try {
    writeStatus_([
      ['Проверка Supabase', 'ВЫПОЛНЯЕТСЯ'],
      ['Начало', startedAt],
      ['Направление', 'Только GET: Supabase → Google Sheets'],
      ['Рабочие вкладки', 'НЕ ИЗМЕНЯЮТСЯ']
    ], '#FFF1C7');

    const result = fetchConnectionProbe_();
    const finishedAt = new Date();

    writeStatus_([
      ['Проверка Supabase', 'СОЕДИНЕНИЕ РАБОТАЕТ'],
      ['Проверено', finishedAt],
      ['HTTP', result.status],
      ['Доступных операций', result.count === null ? 'не сообщил сервер' : result.count],
      ['Получена тестовая строка', result.hasRow ? 'ДА' : 'НЕТ — журнал пока пуст'],
      ['Направление', 'Только GET: Supabase → Google Sheets'],
      ['База и касса', 'НЕ ИЗМЕНЯЛИСЬ']
    ], '#DDF6E5');

    SpreadsheetApp.openById(POTLACH_IMPORT.spreadsheetId).toast(
      'Соединение работает. Результат записан во вкладку «00 — ИНСТРУКЦИЯ».',
      'ПОТЛАЧ',
      8
    );
    ...
    writeFailureStatus_('Проверка соединения', error);
...
/**
 * Загружает операции из Supabase в Google Sheets.
 * Никаких POST/PATCH/DELETE к Supabase в коде нет.
 */
function importPotlachTransactions() {
  ...
  const startedAt = new Date();
  try {
    const ss = SpreadsheetApp.openById(POTLACH_IMPORT.spreadsheetId);
    assertCorrectSpreadsheet_(ss);

    writeStatus_([
      ['Импорт из Supabase', 'ВЫПОЛНЯЕТСЯ'],
      ['Начало', startedAt],
      ['Направление', 'Только GET: Supabase → Google Sheets'],
      ['Supabase и касса', 'НЕ ИЗМЕНЯЮТСЯ']
    ], '#FFF1C7');
    ...
    // До этого места рабочие вкладки не менялись.
    const transactions = fetchAllTransactions_();
    validateTransactions_(transactions);

    const fetchedIds = new Set(transactions.map(function (tx) {
      return String(tx.id);
    }));
    ...
    // Резерв ручной проверки — до очистки или записи рабочих строк.
    ...
    writeImportSummary_(summary, importedAt);
    SpreadsheetApp.flush();

    ss.toast(
      'Импортировано операций: ' + summary.count + '. Supabase не изменялся.',
      'ПОТЛАЧ',
      8
    );

    console.log(JSON.stringify({ok: true, summary: summary}));
    return summary;
  } catch (error) {
    writeFailureStatus_('Импорт остановлен', error);
    console.error(error && error.stack ? error.stack : error);
    throw error;
  } finally {
    lock.releaseLock();
...
/** Короткий запрос максимум одной строки. */
function fetchConnectionProbe_() {
  const fields = 'id';
  ...
  if (!Array.isArray(rows)) {
    throw new Error('Supabase вернул не список операций.');
  }
...
function fetchAllTransactions_() {
  ...
  while (true) {
    ...
    if (offset >= POTLACH_IMPORT.maxRows) {
      throw new Error(
        'Операций больше защитного лимита ' + POTLACH_IMPORT.maxRows + '. Импорт остановлен.'
      );
    }
  }
...
function writeReviewSnapshot_(sheet, transactions) {
  ...
  existing.ids.forEach(function (id) {
    if (!fetchedIds.has(id)) {
      throw new Error(
        'Операция ' + id + ' есть в проверке, но отсутствует в Supabase. Импорт остановлен.'
      );
    }
  });
  ...
  // Формулы S:V не трогаются.
  sheet.getRange(2, 1, oldLastRow - 1, 18).clearContent();
  sheet.getRange(2, 23, oldLastRow - 1, 1).clearContent();
  if (!transactions.length) return;
  ...
  transactions.forEach(function (tx) {
    const id = String(tx.id);
    const prior = existing.reviews.get(id) || ['Новая', '', '', '', '', '', ''];
...
function readExistingReviews_(sheet) {
  const result = {ids: new Set(), reviews: new Map()};
  ...
  values.forEach(function (row) {
    ...
    result.ids.add(id);
    result.reviews.set(id, row.slice(11, 18));
  });
...
function backupReviewSheet_(ss, sourceSheet) {
  ...
  if (!backup) backup = ss.insertSheet(POTLACH_IMPORT.backupSheet);

  const lastRow = Math.max(sourceSheet.getLastRow(), 1);
  const data = sourceSheet
    .getRange(1, 1, lastRow, REVIEW_HEADERS.length)
    .getValues();

  ensureSheetRows_(backup, data.length + 3);
  ensureSheetColumns_(backup, REVIEW_HEADERS.length);
  backup.clearContents();
  backup.getRange('A1').setValue(
    'Автоматический резерв вкладки «02 — ПРОВЕРКА» перед импортом'
  );
...
function assertExistingRowsStillPresent_(sheet, fetchedIds, context) {
  ...
  ids.forEach(function (row) {
    ...
    if (id && !fetchedIds.has(id)) {
      throw new Error(
        'Операция ' + id + ' есть в ' + context +
        ', но отсутствует в новой выгрузке. Ничего не изменено.'
      );
    }
  });
}
  ...
  for (let i = 0; i < expected.length; i += 1) {
    if (actual[i] !== expected[i]) {
      throw new Error(
        ...
        (i + 1) + ' должна называться «' + expected[i] +
        '», сейчас «' + actual[i] + '». Импорт остановлен.'
      );
    }
  }
}
...
function writeImportSummary_(summary, importedAt) {
  writeStatus_([
    ['Импорт из Supabase', 'ЗАВЕРШЁН'],
  ...
  const start = POTLACH_IMPORT.statusStartRow;
  sheet.getRange(start + 1, 2).setNumberFormat('dd.MM.yyyy HH:mm:ss');
  sheet.getRange(start + 4, 2, 2, 1).setNumberFormat('#,##0 [$₽-ru-RU]');
}

function writeFailureStatus_(title, error) {
  ...
  try {
    writeStatus_([
      ...
      ['Время', new Date()],
      ['Причина', message],
      ['Supabase и касса', 'НЕ ИЗМЕНЯЛИСЬ']
    ], '#FDE2E2');
  } catch (statusError) {
...
function writeStatus_(rows, background) {
  const ss = SpreadsheetApp.openById(POTLACH_IMPORT.spreadsheetId);
  assertCorrectSpreadsheet_(ss);
  const sheet = requireSheet_(ss, POTLACH_IMPORT.instructionSheet);
  const start = POTLACH_IMPORT.statusStartRow;

  sheet.getRange(start, 1, 12, 3).clearContent().clearFormat();
  sheet.getRange(start, 1, rows.length, 2).setValues(rows);
  sheet.getRange(start, 1, 1, 2)
    .setFontWeight('bold')
    .setBackground(background || '#F3F3EF');
  sheet.getRange(start, 1, rows.length, 1).setFontWeight('bold');
  sheet.getRange(start, 1, rows.length, 2).setWrap(true);

  rows.forEach(function (row, index) {
    if (row[1] instanceof Date) {
      sheet.getRange(start + index, 2).setNumberFormat('dd.MM.yyyy HH:mm:ss');
    }
  });
